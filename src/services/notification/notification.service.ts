...
const addNormalNotificationMsgTODatabase = async (body: notification,) => {
    ...
    //Preventing null objects ==>
    const filterdRecipients = body.allianceData.students.filter(student => student)
    return new notificationModel({ userId: body.user.id, allianceId: body.allianceData._id, title: body.title, description:body.description, recipient: filterdRecipients, type: body.type, allianceName: body.allianceData.name, userName: body.user.userName,notificationImageUrl:body.user.pfpUrl}).save()
